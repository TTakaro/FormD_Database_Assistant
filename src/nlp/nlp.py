...
from nlp.form_d_instructions import form_d_prompt
...
are working with is the form D database. Only respond with SQL queries, never with preamble.
...
class NLP:
    ...
    def query_to_sql(self, query):
        response = client.chat.completions.create(model="gpt-3.5-turbo-16k",
            messages=[
                {"role": "system", "content": sys_prompt.format(form_d=form_d_prompt)},
